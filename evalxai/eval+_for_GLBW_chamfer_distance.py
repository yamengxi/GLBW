...
def myIOU(preds, labels, reduction='mean'):
    preds = torch.from_numpy(preds[0:1,:,:])
    labels = torch.from_numpy(labels[0:1,:,:])
    ...
    if reduction == 'none':
        return losses
    if reduction == 'mean':
        return losses.mean().numpy()
    if reduction == 'sum':
        return losses.sum()


import os
folder = f'./evalxai+_for_GLBW_experiments_chamfer_distance/{model_path.split("/")[-2]}_{trigger_name}'
