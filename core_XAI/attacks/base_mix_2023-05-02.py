...
class BaseMix(object):
    ...
    def get_adv_mask(self, data_loader, device, log, adv_id, adv_lambda, init=False):
        ...
        if init:
            atanh_m_[m >= 1.0] = -1000000000000.0
        ...
        for epoch in range(self.current_schedule['adv_epochs']):
            ...
            for batch_id, batch in enumerate(data_loader):
                ...
                # losses = backdoor_losses + adv_lambda * (2*penalty_loss - distance)

                # losses = backdoor_losses + 0.0008 * penalty_loss - 0.0001 * distance

                # get multifarious extremums
                # if epoch < 3:
                #     losses = backdoor_losses + adv_lambda * penalty_loss + 2 * adv_lambda * distance_penalty_loss
                # else:
                #     losses = backdoor_losses + adv_lambda * penalty_loss + 2 * adv_lambda * distance_penalty_loss + penalty_loss2
                ...
                # if batch_id == 0:
                #     log(f'==========adv_id-epoch:{adv_id}-{epoch}==========\n')
                #     log(f"now_mean_total_loss:{loss.cpu().item()}, now_mean_ce_loss:{backdoor_losses.mean().cpu().item()}, now_mean_norm_loss:{penalty_loss[0].cpu().item()}, now_mean_distance:{distance[0].cpu().item()}\n")
            ...
            # adjust adv_lambda
            # if epoch == 1 and mean_norm_loss.cpu().item() > 2.0 * m.sum():
            #     return self.get_adv_mask(data_loader, device, log, adv_id, adv_lambda * 2.0)
            ...
            if epoch >= self.current_schedule['adv_epochs'] / 2 and best_mean_total_loss >= mean_total_loss:
                ...
                if best_m.sum() > m.sum() * 2.0:
                    log(f'Error: The best mask is too big!\n')
                    return self.get_adv_mask(data_loader, device, log, adv_id, adv_lambda * 2, init=init)

                if 'poisoned_mean_loss' in self.current_schedule and best_mean_ce_loss > self.current_schedule['poisoned_mean_loss']:
    ...
    def _test(self, dataset, device, batch_size=16, num_workers=8, model=None, test_loss=None, log=None, test_id=None):
        ...
        _, adv_mask, adv_delta, distance = self.get_adv_mask(adv_test_loader, device, log, test_id, self.current_schedule['adv_lambda'])
        probability = probability_activation_function(distance) # (1)
        ...
        with torch.no_grad():
            ...
            for batch_id, batch in enumerate(test_loader):
                ...
                adv_batch_digits = model(adv_batch_imgs)

                batch_digits = torch.cat([benign_batch_digits, poisoned_batch_digits, adv_batch_digits], dim=0) # (3*B, self.num_classes)
