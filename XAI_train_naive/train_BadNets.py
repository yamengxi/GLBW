...
import core
from core_XAI.attacks.base_mix_naive import BaseMix
...
parser.add_argument('--balance_point', default=8.0, type=float)
parser.add_argument('--scale_factor', default=2.0, type=float)
...
probability_activation_function = ProbabilityActivationFunction('sigmoid', balance_point, scale_factor)
...
schedule = {
    ...
    'distance': norm,
    'distance_v2': norm,
    'probability_activation_function': probability_activation_function,
    'adv_lr': 0.1,
    'adv_betas': (0.5, 0.9),
    ...
    'save_dir': f'naive_adv_experiments/{model_path.split("/")[-2]}',
    'experiment_name': f'adv_epochs{adv_epochs}_adv_lambda{0.001 * 9 / trigger_pixel_num * init_cost_rate:.10f}_global_seed{global_seed}_deterministic{deterministic}'
}
...
R = balance_point*2
step = (R-L)/100
x = np.arange(L, R+step, step)
y = probability_activation_function(torch.from_numpy(x)).numpy()
import matplotlib.pyplot as plt
plt.plot(x, y)
os.makedirs(f'naive_adv_experiments/{model_path.split("/")[-2]}', exist_ok=True)
plt.savefig(osp.join(f'naive_adv_experiments/{model_path.split("/")[-2]}', f"probability_activation_function.png"))
plt.close()
