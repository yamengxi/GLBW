...
if trigger_path.endswith('.png'):
    ...
    trigger_pixel_num = (trigger_img > (0.1/255.0)).sum()
...
import os
folder = f'./evalxai++_experiments/evalxai++_{model_path.split("/")[-2]}_{trigger_name}'
...
for i in range(len(L)):
    ...
    for j in range(len(l)):
        ...
        img = l[j].sum(dim=0).detach().cpu().numpy()
        max_pixel_value = 1000
        img = img / img.max() * max_pixel_value
        img = img.round().astype(np.uint16)
        img = cv2.threshold(img, 0, max_pixel_value, cv2.THRESH_OTSU)
        evalxai_mask = img[1].astype(np.float32) / max_pixel_value
        evalxai_mask = np.stack([evalxai_mask.copy() for i in range(3)], axis=0)
