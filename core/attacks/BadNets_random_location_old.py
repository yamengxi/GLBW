...
class AddTrigger:
    ...
    def add_trigger(self, img):
        ...
        """
        if img.numel() != self.res.numel():
            _, H, W = img.shape
            _, h_size, w_size = self.res.shape
            h_pos = random.randint(0, H-h_size)
            w_pos = random.randint(0, W-w_size)
            img[:, h_pos:h_pos+h_size, w_pos:w_pos+w_size] = self.weight * img[:, h_pos:h_pos+h_size, w_pos:w_pos+w_size] + self.res
            return img
        else:
...
class AddDatasetFolderTrigger(AddTrigger):
    ...
    def __init__(self, pattern, weight, random_location):
        ...
        if random_location:
            tmp = torch.nonzero(self.weight)
            w1, w2 = tmp[:, 2].min(), tmp[:, 2].max()
            h1, h2 = tmp[:, 1].min(), tmp[:, 1].max()
            self.weight = self.weight[:, h1:h2+1, w1:w2+1]
            self.pattern = self.pattern[:, h1:h2+1, w1:w2+1]
...
class PoisonedDatasetFolder(DatasetFolder):
    def __init__(self,
        ...
        self.poisoned_transform.transforms.insert(poisoned_transform_index, AddDatasetFolderTrigger(pattern, weight, random_location))
...
class PoisonedMNIST(MNIST):
    def __init__(self,
        ...
        if self.transform is None:
            ...
            self.poisoned_transform = copy.deepcopy(self.transform)
        self.poisoned_transform.transforms.insert(poisoned_transform_index, AddMNISTTrigger(pattern, weight, random_location))
...
class PoisonedCIFAR10(CIFAR10):
    def __init__(self,
        ...
        if self.transform is None:
            ...
            self.poisoned_transform = copy.deepcopy(self.transform)
        self.poisoned_transform.transforms.insert(poisoned_transform_index, AddCIFAR10Trigger(pattern, weight, random_location))
...
def CreatePoisonedDataset(benign_dataset, y_target, poisoned_rate, pattern, weight, random_location, poisoned_transform_index, poisoned_target_transform_index):
    class_name = type(benign_dataset)
    if class_name == DatasetFolder:
        return PoisonedDatasetFolder(benign_dataset, y_target, poisoned_rate, pattern, weight, random_location, poisoned_transform_index, poisoned_target_transform_index)
    elif class_name == MNIST:
        return PoisonedMNIST(benign_dataset, y_target, poisoned_rate, pattern, weight, random_location, poisoned_transform_index, poisoned_target_transform_index)
    elif class_name == CIFAR10:
        return PoisonedCIFAR10(benign_dataset, y_target, poisoned_rate, pattern, weight, random_location, poisoned_transform_index, poisoned_target_transform_index)
    else:
