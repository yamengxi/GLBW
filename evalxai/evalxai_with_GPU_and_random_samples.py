...
# In[4]:
from test_module.imagenet_config_random_samples import Config as imagenet_config
from test_module.test_module import TestModule

# Initialize test module
test_module = TestModule(exp_name, imagenet_config(model_name, model_path, data_path, exp_name, trigger_size))
...
import os
folder = f'{args.save_dir}/saliency_maps/'+model_name+'_'+exp_name+'_'+str(trigger_size)
...
for i in range(len(L)):
    ...
    for j in range(len(l)):
        g = normalize(l[j].detach().cpu().numpy()) # rerange to [0, 1]
        if g.shape[0] == 1:
            g = g.repeat(3, 0)
