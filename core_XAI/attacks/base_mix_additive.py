...
class AddTrigger:
    ...
    def __init__(self, num_classes, y_target, addition, clip_min, clip_max, device):
        self.num_classes = num_classes
        self.y_target = y_target
        self.addition = addition.to(device)
    ...
    def __call__(self, imgs):
        ...
        """
        poisoned_imgs = (imgs + self.addition).clip(min=self.clip_min, max=self.clip_max)
...
class BaseMix(object):
    ...
    def __init__(self, train_dataset, test_dataset, model, loss, num_classes, y_target, addition, schedule=None, seed=0, deterministic=False):
    ...
    def get_adv_add(self, data_loader, device, log, adv_id, adv_lambda, init=False):
        add = self.addition.to(device)
        if add.dim() == 2:
            add = add.unsqueeze(0)
        add = add.unsqueeze(0) # (1, 1, H, W)
        ...
        atanh_add_ = torch.randn(add.shape, device=device) # (1, 1, H, W)
        if init:
            atanh_add_[add > 0.0] = -1000000000000.0
        atanh_add_ = torch.nn.Parameter(atanh_add_)


        optimizer = torch.optim.Adam([atanh_add_], lr=self.current_schedule['adv_lr'], betas=self.current_schedule['adv_betas'])
        ...
        for epoch in range(self.current_schedule['adv_epochs']):
            if epoch in self.current_schedule['adv_schedule']:
                optimizer = torch.optim.SGD([atanh_add_], lr=2048.0, momentum=0.0, weight_decay=0.0)

            sum_ce_loss = 0.0
            sum_norm_loss = 0.0
            for batch_id, batch in enumerate(data_loader):
                # Map to [0.0, 1.0] (differentiable).
                add_ = tanh_func(atanh_add_) # (1, 1, H, W), [0.0, 1.0]
                add_ = self.current_schedule['normalize'](add_) # (1, C, H, W), [-x, +y]
                ...
                digits = f((x + add_).clip(min=self.current_schedule['clip_min'], max=self.current_schedule['clip_max'])) # (B, self.num_classes)

                backdoor_losses = L(digits, y_t) # (B), minimum

                distance = (add_ - add).abs().sum(dim=(1, 2, 3)) # (1)

                penalty_loss = add_.abs().sum(dim=(1, 2, 3)) # (1)
            ...
            cv2.imwrite(osp.join(log.log_path[:-7], str(adv_id).zfill(4), str(batch_id).zfill(4), str(epoch).zfill(4) + '_add.png'), (add_[0].permute(1, 2, 0) * 255).round().clip(0, 255).to(dtype=torch.uint8).cpu().numpy())
            ...
            adv_img0=self.data2img(x[0] + add_[0])
            ...
            if epoch >= min(3, self.current_schedule['adv_epochs'] / 2) and best_mean_total_loss >= mean_total_loss:
                ...
                best_add = add_.detach() # best addition
                best_distance = (add_ - add).abs().sum(dim=(1, 2, 3)).detach() # best mask, (1)

                sqrt_add = (add.abs().sum() ** 0.5).item()
                a = (np.log(2) / np.log(7/3)).item()
                b = 3 ** a
                if best_add.abs().sum() > add.abs().sum() * max(1 + b / sqrt_add**a, 1.3):
                    log(f'Error: {best_add.abs().sum().item()} > {add.abs().sum().item() * max(1 + b / sqrt_add**a, 1.3)}, the best mask is too big!\n')
                    return self.get_adv_add(data_loader, device, log, adv_id, adv_lambda * best_add.abs().sum() / add.abs().sum(), init=init)

                if best_add.abs().sum() < add.abs().sum() * 0.6 and 'poisoned_mean_loss' in self.current_schedule and best_mean_ce_loss > 1.8 * self.current_schedule['poisoned_mean_loss']:
                    log(f'Error: The best mask is too small!\n')
                    return self.get_adv_add(data_loader, device, log, adv_id, adv_lambda * 0.618, init=init)

                if 'poisoned_mean_loss' in self.current_schedule and (best_mean_ce_loss + adv_lambda * best_add.abs().sum() > 1.5 *(self.current_schedule['poisoned_mean_loss'] + adv_lambda * add.abs().sum()) or best_mean_ce_loss > 1.8 * self.current_schedule['poisoned_mean_loss']):
                    log(f'Error: We got a bad mask!\n')
                    return self.get_adv_add(data_loader, device, log, adv_id, adv_lambda, init=init)

                if (best_add * add).sum() > add.abs().sum() * 0.05:
                    log(f'Error: The intersection of the best mask and the standard mask is too big!\n')
                    return self.get_adv_add(data_loader, device, log, adv_id, adv_lambda, init=True)
        ...
        cv2.imwrite(osp.join(log.log_path[:-7], str(adv_id).zfill(4), 'best_mask.png'), (best_add[0].permute(1, 2, 0) * 255).round().clip(0, 255).to(dtype=torch.uint8).cpu().numpy())
        ...
        best_adv_img0=self.data2img(x[0] + best_add[0])
        cv2.imwrite(osp.join(log.log_path[:-7], str(adv_id).zfill(4), 'best_adv_img0.png'), cv2.cvtColor(best_adv_img0, cv2.COLOR_RGB2BGR))


        return best_mean_total_loss.cpu().item(), best_add, best_distance # float, (1, 1, H, W), (1)
    ...
    def train(self, schedule=None):
        ...
        # self.model.eval()
        # for i in range(100):
        #     _, adv_mask, adv_delta, distance = self.get_adv_add(train_loader, device, log, 999, self.current_schedule['adv_lambda'])
        # self.model.train()
        # breakpoint()
        ...
        add_trigger = AddTrigger(self.num_classes, self.y_target, self.addition, self.current_schedule["clip_min"], self.current_schedule["clip_max"], device)
        ...
        for i in range(self.current_schedule['epochs']):
            ...
            if poisoned_test_dataset_accuracy > -1.0:
                self.model.eval()
                _, adv_add, distance = self.get_adv_add(train_loader, device, log, i+1, self.current_schedule['adv_lambda'])
                self.model.train()
                ...
                for batch_id, batch in enumerate(train_loader):
                    ...
                    adv_imgs = (imgs + adv_add).clip(min=self.current_schedule['clip_min'], max=self.current_schedule['clip_max']) # (B, C, H, W)
