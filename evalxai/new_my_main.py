...
def parse():
    ...
    parser.add_argument("--save_name", default='No_Name', type=str)
...
def main():
    ...
    if args.test:
        ...
        print("Test mode - no DDP, no apex, RN50, 10 iterations", flush=True)
    ...
    print("opt_level = {}".format(args.opt_level), flush=True)
    print("keep_batchnorm_fp32 = {}".format(args.keep_batchnorm_fp32), type(args.keep_batchnorm_fp32), flush=True)
    print("loss_scale = {}".format(args.loss_scale), type(args.loss_scale), flush=True)

    print("\nCUDNN VERSION: {}\n".format(torch.backends.cudnn.version()), flush=True)
    ...
    # create model
    print(f"=> creating model {args.arch}, using pre-trained model weights {args.weights}", flush=True)
    model = models.__dict__[args.arch](weights=args.weights, num_classes=args.num_classes)

    if args.sync_bn:
        print("using apex synced BN", flush=True)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        val_loader.reset()
        print("Test on dirty images:", flush=True)
        [prec1_trig, prec5_trig] = test(val_loader, model, criterion, is_trig=True, one_time_test=False)
        ...
        if args.local_rank == 0:
            ...
            if epoch == args.epochs - 1:
                print('##Top-1 {0}\n'
                      ...
                      prec1,
                      prec5,
                      args.total_batch_size / total_time.avg), flush=True)
...
def get_images(dname,normalize=True):
    ...
    if os.path.exists(dname):
        ...
        Warning(f"{dname} does not exist!")
    return textures
...
def addTrigger(input, target, portion):
    ...
    elif args.trig.startswith("fixed") or args.trig.startswith("none"):
        pi = pj = 224-d
    ...
    if args.trig == 'location':
        ...
        target[x] = y
    elif args.trig.endswith('square') or args.trig.endswith('none'):
    ...
    '''
    if portion == 1:
        print("Target Label:", y, flush=True)
    '''
...
def train(train_loader, model, criterion, optimizer, epoch, val_loader):
    ...
    for i, data in enumerate(train_loader):
        ...
        if args.prof >= 0 and i == args.prof:
            print("Profiling begun at iteration {}".format(i), flush=True)
            ...
            if args.local_rank == 0:
                print('{0}\t'
                      ...
                      'Prec@5 {top5.val:.3f} ({top5.avg:.3f})'.format(
                       ...
                       loss=losses, top1=top1, top5=top5), flush=True)
        ...
        if args.prof >= 0 and i == args.prof + 10:
            print("Profiling ended at iteration {}".format(i), flush=True)
