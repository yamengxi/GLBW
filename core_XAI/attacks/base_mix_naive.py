...
class BaseMix(object):
    ...
    def get_adv_mask(self, data_loader, device, log, adv_id, adv_lambda, init=False):
        ...
        best_mean_adv_loss = float("inf")
        best_mean_benign_loss = float("inf")
        best_mean_ce_loss = float("inf")
        best_mean_L1_norm_loss = float("inf")
        best_m = None
        best_delta = None
        best_distance = None
        for epoch in range(self.current_schedule['adv_epochs']):
            ...
            for batch_id, batch in enumerate(data_loader):
                ...
                probabilities = probability_activation_function(distance) # (1)

                adv_loss = -cal_adv_loss(L, digits, y, y_t, probabilities) # (B)
                benign_loss = L(digits, y) # (B)
                ce_loss = L(digits, y_t) # (B)
                L1_norm_loss = (m_[:,0,:,:].abs()).sum(dim=(1, 2)) # (1)
                sum_adv_loss += adv_loss.sum() # scalar tensor
                sum_benign_loss += benign_loss.sum() # scalar tensor
                sum_ce_loss += ce_loss.sum() # scalar tensor
                sum_L1_norm_loss += L1_norm_loss[0] * B # scalar tensor

                penalty_loss = torch.maximum(torch.tensor([0.0]).cuda(), L1_norm_loss - 18.0)
                # penalty_loss = torch.maximum(torch.zeros_like(distance), distance - 27.0)
                # penalty_loss = 0.0
                final_loss = adv_loss + penalty_loss
            ...
            log(f"epoch last distance:{distance[0].cpu().item()}, epoch last probabilities: {probabilities[0].cpu().item()}\n")
            mean_adv_loss = sum_adv_loss / len(data_loader.dataset) # scalar tensor
            mean_benign_loss = sum_benign_loss / len(data_loader.dataset) # scalar tensor
            mean_ce_loss = sum_ce_loss / len(data_loader.dataset) # scalar tensor
            mean_L1_norm_loss = sum_L1_norm_loss / len(data_loader.dataset) # scalar tensor
            log(f"{time.strftime('[%Y-%m-%d_%H:%M:%S]', time.localtime())} mean_adv_loss:{mean_adv_loss.cpu().item()}, mean_benign_loss:{mean_benign_loss.cpu().item()}, mean_ce_loss:{mean_ce_loss.cpu().item()}, mean_L1_norm_loss: {mean_L1_norm_loss.cpu().item()}\n")
            ...
            if epoch >= min(3, self.current_schedule['adv_epochs'] / 2) and best_mean_adv_loss >= mean_adv_loss:
                best_mean_adv_loss = mean_adv_loss
                best_mean_benign_loss = mean_benign_loss
                best_mean_ce_loss = mean_ce_loss
                best_mean_L1_norm_loss = mean_L1_norm_loss
        ...
        log(time.strftime("[%Y-%m-%d_%H:%M:%S] ", time.localtime()) + f"best_mean_adv_loss:{best_mean_adv_loss.cpu().item()}, best_mean_benign_loss:{best_mean_benign_loss.cpu().item()}, best_mean_ce_loss:{best_mean_ce_loss.cpu().item()}, best_mean_L1_norm_loss:{best_mean_L1_norm_loss.cpu().item()}, best_distance:{best_distance[0].cpu().item()}\n")
        ...
        return best_mean_adv_loss.cpu().item(), best_m, best_delta, best_distance # float, (1, 1, H, W), (1, C, H, W), (1)
