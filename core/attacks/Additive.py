'''
This is the implement of Additive [1].
...
class AddTrigger:
    ...
    def add_trigger(self, img):
        ...
        """
        return (img + self.addition).clip(min=self.clip_min, max=self.clip_max)
...
class AddDatasetFolderTrigger(AddTrigger):
    ...
    def __init__(self, addition, clip_min, clip_max):
        super(AddDatasetFolderTrigger, self).__init__()

        if addition is None:
            raise ValueError("addition can not be None.")
        else:
            self.addition = addition
            if self.addition.dim() == 2:
                self.addition = self.addition.unsqueeze(0)
...
class AddMNISTTrigger(AddTrigger):
    ...
    def __init__(self, addition, clip_min, clip_max):
        ...
        if addition is None:
            self.addition = torch.zeros((1, 28, 28), dtype=torch.uint8)
            self.addition[0, -2:, -2:] = 255
        else:
            self.addition = addition
            if self.addition.dim() == 2:
                self.addition = self.addition.unsqueeze(0)
...
class AddCIFAR10Trigger(AddTrigger):
    ...
    def __init__(self, addition, clip_min, clip_max):
        ...
        if addition is None:
            self.addition = torch.zeros((1, 32, 32), dtype=torch.uint8)
            self.addition[0, -3:, -3:] = 255
        else:
            self.addition = addition
            if self.addition.dim() == 2:
                self.addition = self.addition.unsqueeze(0)
...
class PoisonedDatasetFolder(DatasetFolder):
    def __init__(self,
        ...
        self.poisoned_transform.transforms.insert(poisoned_transform_index, AddDatasetFolderTrigger(addition, clip_min, clip_max))
...
class PoisonedMNIST(MNIST):
    def __init__(self,
        ...
        if self.transform is None:
            ...
            self.poisoned_transform = copy.deepcopy(self.transform)
        self.poisoned_transform.transforms.insert(poisoned_transform_index, AddMNISTTrigger(addition, clip_min, clip_max))
...
class PoisonedCIFAR10(CIFAR10):
    def __init__(self,
        ...
        if self.transform is None:
            ...
            self.poisoned_transform = copy.deepcopy(self.transform)
        self.poisoned_transform.transforms.insert(poisoned_transform_index, AddCIFAR10Trigger(addition, clip_min, clip_max))
...
def CreatePoisonedDataset(benign_dataset, y_target, poisoned_rate, addition, clip_min, clip_max, poisoned_transform_index, poisoned_target_transform_index):
    class_name = type(benign_dataset)
    if class_name == DatasetFolder:
        return PoisonedDatasetFolder(benign_dataset, y_target, poisoned_rate, addition, clip_min, clip_max, poisoned_transform_index, poisoned_target_transform_index)
    elif class_name == MNIST:
        return PoisonedMNIST(benign_dataset, y_target, poisoned_rate, addition, clip_min, clip_max, poisoned_transform_index, poisoned_target_transform_index)
    elif class_name == CIFAR10:
        return PoisonedCIFAR10(benign_dataset, y_target, poisoned_rate, addition, clip_min, clip_max, poisoned_transform_index, poisoned_target_transform_index)
    else:
        raise NotImplementedError


class Additive(Base):
    """Construct poisoned datasets with Additive method.
    ...
    def __init__(self,
        ...
        self.addition = addition.clone()
        self.weight = addition.clone()
        self.weight[self.weight > 0.0] = 0.99
        self.pattern = addition.clone() * 100
