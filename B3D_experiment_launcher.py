...
from B3D.B3D import optimize_blackbox


parser = argparse.ArgumentParser(description='B3D Experiments Launcher for Trojaned Models on Datasets')
...
parser.add_argument('--B3D_epochs', default=10, type=int)
parser.add_argument('--B3D_lr', default=0.1, type=float)
parser.add_argument('--attack_succ_threshold', default=0.99, type=float)
parser.add_argument('--samples_per_draw', default=50, type=int)
parser.add_argument('--save_trigger_path', default='./B3D/now_B3D_experiments', type=str)
...
save_trigger_path=osp.join(save_trigger_path, f"{model_name}_{dataset_name}_{trigger_size}x{trigger_size}_epochs{B3D_epochs}_lr{B3D_lr:.5f}_init_cost{0.001 * 9 / (trigger_size * trigger_size) * init_cost_rate:.10f}_samples_per_draw{samples_per_draw}_{time.strftime('%Y-%m-%d_%H:%M:%S', time.localtime())}")
...
if dataset_name=='MNIST':
    ...
    testset_with_transform_train = dataset(dataset_root_path, train=False, transform=transform_train, download=True)
    ...
    testset = DatasetFolder(
        ...
        target_transform=None,
        is_valid_file=None)
    testset_with_transform_train = DatasetFolder(
        root=osp.join(dataset_root_path, 'GTSRB', 'testset'),
        loader=cv2.imread,
        extensions=('png',),
        transform=transform_train,
    ...
    testset_with_transform_train = dataset(
        root=osp.join(dataset_root_path, 'ImageNet_100', 'val'),
        loader=my_read_image,
        extensions=('jpeg',),
        transform=transform_train,
...
while not stop:
    ...
    # mask (1, H, W), pattern (C, H, W)
    mask, pattern = optimize_blackbox(
        model=model,
    ...
    schedule = {
        ...
        'experiment_name': f'B3D_trigger_test_ASR'
    }
    top1_correct, top5_correct, total_num, mean_loss = core.utils.test(model, poisoned_testset, schedule)
...
with open(osp.join(save_trigger_path, f'__summary_experiments_results', 'trigger_statistics.log'),'w') as f:
    f.write(f'lr:{B3D_lr:.10f}\n')
    f.write(f'epochs:{B3D_epochs}\n')
    ...
    f.write(f'{B3D_lr:.10f}\n')
    f.write(f'{B3D_epochs}\n')
