...
if dataset_name=='MNIST':
    ...
    N = 500
    x = torch.stack([transform_test(cv2.imread(item[0])) for item in testset.samples[:N]]) # torch.Tensor torch.float32 [0.0, 1.0] (N, 3, 32, 32)
    y = torch.tensor([item[1] for item in testset.samples[:N]], dtype=torch.int64) # torch.Tensor torch.int64 (N)
    normalize = torch.nn.Identity()
    inv_normalize = torch.nn.Identity()
...
if trigger_img.ndim == 3:
    trigger_pixel_num /= trigger_img.shape[2]
...
for i in range(len(test_data_orig)):
    ...
    img_trig = (1 - std_weight.to(device)) * img_orig + std_weight.to(device) * std_pattern.to(device)
    label_trig = y_target
    trigger = std_weight
...
def get_saliency_maps(img, label, model):
    ...
    if dataset_name == "CIFAR-10" or dataset_name == "GTSRB":
        ...
        m.append(torch.nn.Upsample(scale_factor=16, mode='nearest'))
        mask.append(torch.arange(0, 196, dtype=torch.float32).view(1, 1, 14, 14).to(device=device))
        m.append(torch.nn.Upsample(scale_factor=32, mode='nearest'))
        mask.append(torch.arange(0, 49, dtype=torch.float32).view(1, 1, 7, 7).to(device=device))
...
import os
folder = f'./evalxai_experiments/evalxai_{model_path.split("/")[-2]}'
...
for i in range(len(L)):
    ...
    for j in range(len(l)):
        g = rerange(l[j].detach().cpu().numpy())
        if g.shape[0] == 1:
            g = g.repeat(3, 0)
