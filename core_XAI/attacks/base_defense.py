...
class BaseDefense(object):
    ...
    def get_adv_mask(self, data_loader, device, log, adv_id, adv_lambda, init=False):
        ...
        if init:
            atanh_m_[m > 0.0] = -1000000000000.0
        ...
        for epoch in range(self.current_schedule['adv_epochs']):
            if epoch in self.current_schedule['adv_schedule']:
                optimizer = torch.optim.SGD([atanh_delta_, atanh_m_], lr=2048.0, momentum=0.0, weight_decay=0.0)
            ...
            for batch_id, batch in enumerate(data_loader):
                ...
                delta_ = self.current_schedule['normalize'](delta_) # (1, C, H, W), [-x, +y]
                ...
                penalty_loss2 = torch.maximum(torch.zeros_like(penalty_loss), penalty_loss - 600.0)
            ...
            log(time.strftime("[%Y-%m-%d_%H:%M:%S] ", time.localtime()) + f"mean_total_loss:{mean_total_loss.cpu().item()}, mean_ce_loss:{mean_ce_loss.cpu().item()}, mean_norm_loss:{mean_norm_loss.cpu().item()}, adv_lambda: {adv_lambda}\n")

            batch_id = len(data_loader) - 1
            os.makedirs(osp.join(log.log_path[:-7], str(adv_id).zfill(4), str(batch_id).zfill(4)), exist_ok=True)
            cv2.imwrite(osp.join(log.log_path[:-7], str(adv_id).zfill(4), str(batch_id).zfill(4), str(epoch).zfill(4) + '_mask.png'), (m_[0].permute(1, 2, 0) * 255).round().to(dtype=torch.uint8).cpu().numpy())
            pattern=(delta_[0].permute(1, 2, 0) * 255).round().to(dtype=torch.uint8).cpu().numpy()
            cv2.imwrite(osp.join(log.log_path[:-7], str(adv_id).zfill(4), str(batch_id).zfill(4), str(epoch).zfill(4) + '_pattern.png'), cv2.cvtColor(pattern, cv2.COLOR_RGB2BGR))
            res=((m_[0] * delta_[0]).permute(1, 2, 0) * 255).round().to(dtype=torch.uint8).cpu().numpy()
            cv2.imwrite(osp.join(log.log_path[:-7], str(adv_id).zfill(4), str(batch_id).zfill(4), str(epoch).zfill(4) + '_res.png'), cv2.cvtColor(res, cv2.COLOR_RGB2BGR))
            img0=self.data2img(x[0])
            cv2.imwrite(osp.join(log.log_path[:-7], str(adv_id).zfill(4), str(batch_id).zfill(4), str(epoch).zfill(4) + '_img0.png'), cv2.cvtColor(img0, cv2.COLOR_RGB2BGR))
            adv_img0=self.data2img(x[0] * (1.0 - m_[0]) + m_[0] * delta_[0])
            cv2.imwrite(osp.join(log.log_path[:-7], str(adv_id).zfill(4), str(batch_id).zfill(4), str(epoch).zfill(4) + '_adv_img0.png'), cv2.cvtColor(adv_img0, cv2.COLOR_RGB2BGR))

            if epoch >= min(3, self.current_schedule['adv_epochs'] / 2) and best_mean_total_loss >= mean_total_loss:
                ...
                best_distance = d(m_[:,0,:,:], m[:,0,:,:]).detach() # best mask, (1)

                sqrt_m = (m.sum() ** 0.5).item()
                a = (np.log(2) / np.log(7/3)).item()
                b = 3 ** a
                if best_m.sum() > m.sum() * max(1 + b / sqrt_m**a, 1.3):
                    log(f'Error: {best_m.sum().item()} > {m.sum().item() * max(1 + b / sqrt_m**a, 1.3)}, the best mask is too big!\n')
                    return self.get_adv_mask(data_loader, device, log, adv_id, adv_lambda * best_m.sum() / m.sum(), init=init)

                if best_m.sum() < m.sum() * 0.6 and 'poisoned_mean_loss' in self.current_schedule and best_mean_ce_loss > 1.8 * self.current_schedule['poisoned_mean_loss']:
                    log(f'Error: The best mask is too small!\n')
                    return self.get_adv_mask(data_loader, device, log, adv_id, adv_lambda * 0.618, init=init)

                # if 'poisoned_mean_loss' in self.current_schedule and (best_mean_ce_loss + adv_lambda * best_m.sum() > 1.5 *(self.current_schedule['poisoned_mean_loss'] + adv_lambda * m.sum()) or best_mean_ce_loss > 1.8 * self.current_schedule['poisoned_mean_loss']):
                #     log(f'Error: We got a bad mask!\n')
                #     return self.get_adv_mask(data_loader, device, log, adv_id, adv_lambda, init=init)

                if (best_m * m).sum() > m.sum() * 0.05:
                    log(f'Error: The intersection of the best mask and the standard mask is too big!\n')
                    return self.get_adv_mask(data_loader, device, log, adv_id, adv_lambda, init=True)

        if not init:
            return self.get_adv_mask(data_loader, device, log, adv_id, adv_lambda, init=False)

        log(f'==========adv_id:{adv_id}-result==========\n')
        log(time.strftime("[%Y-%m-%d_%H:%M:%S] ", time.localtime()) + f"best_mean_total_loss:{best_mean_total_loss.cpu().item()}, best_mean_ce_loss:{best_mean_ce_loss.cpu().item()}, best_mean_norm_loss:{best_mean_norm_loss.cpu().item()}, best_distance:{best_distance[0].cpu().item()}\n")

        cv2.imwrite(osp.join(log.log_path[:-7], str(adv_id).zfill(4), 'best_mask.png'), (best_m[0].permute(1, 2, 0) * 255).round().to(dtype=torch.uint8).cpu().numpy())
        best_pattern=(best_delta[0].permute(1, 2, 0) * 255).round().to(dtype=torch.uint8).cpu().numpy()
        cv2.imwrite(osp.join(log.log_path[:-7], str(adv_id).zfill(4), 'best_pattern.png'), cv2.cvtColor(best_pattern, cv2.COLOR_RGB2BGR))
        best_res=((best_m[0] * best_delta[0]).permute(1, 2, 0) * 255).round().to(dtype=torch.uint8).cpu().numpy()
        cv2.imwrite(osp.join(log.log_path[:-7], str(adv_id).zfill(4), 'best_res.png'), cv2.cvtColor(best_res, cv2.COLOR_RGB2BGR))
        best_img0=self.data2img(x[0])
        cv2.imwrite(osp.join(log.log_path[:-7], str(adv_id).zfill(4), 'best_img0.png'), cv2.cvtColor(best_img0, cv2.COLOR_RGB2BGR))
        best_adv_img0=self.data2img(x[0] * (1.0 - best_m[0]) + best_m[0] * best_delta[0])
        cv2.imwrite(osp.join(log.log_path[:-7], str(adv_id).zfill(4), 'best_adv_img0.png'), cv2.cvtColor(best_adv_img0, cv2.COLOR_RGB2BGR))


        return best_mean_total_loss.cpu().item(), best_m, best_delta, best_distance # float, (1, 1, H, W), (1, C, H, W), (1)
    ...
    def train(self, schedule=None):
        ...
        # self.model.eval()
        # for i in range(100):
        #     _, adv_mask, adv_delta, distance = self.get_adv_mask(train_loader, device, log, 999, self.current_schedule['adv_lambda'])
        # self.model.train()
        # breakpoint()
        ...
        for i in range(self.current_schedule['epochs']):
            if i == 0: # First test!
                ...
                msg = "==========Test result on benign test dataset==========\n" + \
                      ...
                      f"Top-1 correct / Total: {top1_correct}/{total_num}, Top-1 accuracy: {top1_correct/total_num}, Top-5 correct / Total: {top5_correct}/{total_num}, Top-5 accuracy: {top5_correct/total_num}, mean loss: {mean_loss}, time: {time.time()-last_time}\n"
                log(msg)
                benign_test_dataset_accuracy = top1_correct/total_num
            ...
            if benign_test_dataset_accuracy > 0.86:
                self.model.eval()
                ...
                for batch_id, batch in enumerate(train_loader):
                    ...
                    selected_bool_index = (torch.rand_like(labels, dtype=torch.float32) < lambda_1)
                    benign_digits[selected_bool_index] = poisoned_digits[selected_bool_index]
                    labels[selected_bool_index] = poisoned_labels[selected_bool_index]

                    selected_bool_index = (torch.rand_like(labels, dtype=torch.float32) < lambda_2)
                    adv_digits = adv_digits[selected_bool_index]
                    adv_labels = labels[selected_bool_index]
                    poisoned_labels = poisoned_labels[selected_bool_index]

                    losses = [self.loss(benign_digits[~selected_bool_index], labels[~selected_bool_index]), cal_adv_loss(self.loss, adv_digits, adv_labels, poisoned_labels, probability)]
                ...
                for batch_id, batch in enumerate(train_loader):
                    ...
                    # losses = self.loss(benign_digits, labels)

                    if random.random() < lambda_1:
                        losses = self.loss(benign_digits, labels) + self.loss(poisoned_digits, poisoned_labels)
                    else:
                        losses = self.loss(benign_digits, labels)
