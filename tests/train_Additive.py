...
trigger_img[trigger_img>0] = addition_value
addition = torch.from_numpy(trigger_img).to(dtype=torch.float32) / 255.0 # (C, H, W), [0.0, 1.0]
...
schedule = {
    ...
    'experiment_name': f'{model_type}_{model_name}_GTSRB_Additive_addition_value={addition_value}_{trigger_name}_random_location={random_location}_poisoning_rate={poisoning_rate}_y_target={y_target}'
}
additive.train(schedule)
